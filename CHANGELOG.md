...
- 🔴 **Critical Fix**: 修復 dashboard 路由衝突並恢復專業 Layout
  - **問題**: `01-base/app/dashboard/` 與 `01-base/app/(demo)/dashboard/` 路由衝突
  ...
  - **解決方案**:
    - 刪除 `01-base/app/dashboard/` 目錄（避免衝突）
    - 修改 `app/(demo)/dashboard/page.tsx` 使用 `DashboardLayout` 組件
...
### 修改的文件 (Changed)
- `01-base/app/(demo)/dashboard/page.tsx.template` - 整合 DashboardLayout 組件，移除自定義 header
...
- ⚠️ **注意**: 如果您已創建的項目遇到此問題：
  ...
  2. 用模板的最新版本替換 `app/(demo)/dashboard/page.tsx`
